...
# 设置基础路径
MAP_FILE="../assets/${MAP_NAME}.map"
INSTANCE_BASE="../assets/${MAP_NAME}"
...
for AGENTS in "${AGENTS_LIST[@]}"; do
    # 创建输出目录
    OUTPUT_DIR="${OUTPUT_BASE}/${AGENTS}_origin"
    mkdir -p $OUTPUT_DIR

    # 创建CSV文件并写入表头
    echo "Instance,Solved,comp_time,soc,makespan" > "$OUTPUT_DIR/summary.csv"

    # 运行25个算例
    for i in {1..25}
    do
        # 构建文件路径
        INPUT_FILE="${INSTANCE_BASE}/${MAP_NAME}-random-$i.scen"
        ...
        # 运行程序
        ./main -m $MAP_FILE -i $INPUT_FILE -N $AGENTS -v 2 -t 20 -o $OUTPUT_FILE --objective 1
        ...
        if [ $? -eq 0 ]; then
            ...
            RESULT=$(awk '
                ...
                END {
                    ...
                    split(soc, sc, "=");
                    split(makespan, m, "=");
                    printf "%d,%d,%d,%d", s[2], c[2], sc[2], m[2]
                }' "$OUTPUT_FILE")
